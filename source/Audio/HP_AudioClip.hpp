/* HP_AudioClip.hpp -- Implementation of the API for managing audio clips
 *
...
#ifndef HP_AUDIO_CLIP_HPP
#define HP_AUDIO_CLIP_HPP
...
public:
    HP_AudioClip(const char* filePath, int channelCount);
    ~HP_AudioClip();

    HP_AudioClip(const HP_AudioClip&) = delete;
    HP_AudioClip& operator=(const HP_AudioClip&) = delete;

    HP_AudioClip(HP_AudioClip&&) = delete;
    HP_AudioClip& operator=(HP_AudioClip&&) = delete;
...
inline HP_AudioClip::~HP_AudioClip()
{
    if (mBuffer > 0) {
...
inline bool HP_AudioClip::isValid() const
{
    return mBuffer > 0;
}

inline int HP_AudioClip::play(int channel)
{
    channel = HP_MIN(channel, static_cast<int>(mSources.size() - 1));
...
inline void HP_AudioClip::pause(int channel)
{
...
inline void HP_AudioClip::stop(int channel)
{
    if (channel >= static_cast<int>(mSources.size())) return;
...
inline void HP_AudioClip::rewind(int channel)
{
    if (channel >= static_cast<int>(mSources.size())) return;
...
inline bool HP_AudioClip::isPlaying(int channel) const
{
...
inline int HP_AudioClip::getChannelCount() const
{
...
inline HP_AudioClip::RawData HP_AudioClip::loadWAV(const void* data, size_t data_size)
{
    RawData result;
    drwav wav;

    if (!drwav_init_memory(&wav, data, data_size, nullptr)) {
        HP_INTERNAL_LOG(E, "AUDIO: Failed to initialize WAV decoder");
        return result;
    }
    ...
    if (wav.channels == 1 && wav.bitsPerSample == 16) {
        ...
        HP_INTERNAL_LOG(E, "AUDIO: Unsupported WAV format (channels: %u, bits: %u)", wav.channels, wav.bitsPerSample);
        drwav_uninit(&wav);
        return result;
    }
    ...
    if (!result.pcmData) {
        HP_INTERNAL_LOG(E, "AUDIO: Failed to allocate memory for PCM data");
        drwav_uninit(&wav);
        result = {};
        return result;
    }
    ...
    if (frames_read != totalFrames) {
        HP_INTERNAL_LOG(E, "AUDIO: Failed to read all PCM frames");
...
inline HP_AudioClip::RawData HP_AudioClip::loadFLAC(const void* data, size_t data_size)
{
    RawData result;
    ...
    if (!pcmData) {
        HP_INTERNAL_LOG(E, "AUDIO: Failed to decode FLAC file");
        return result;
    }
    ...
    if (channels == 1) {
        ...
        HP_INTERNAL_LOG(E, "AUDIO: Unsupported number of channels (%u) in FLAC file", channels);
...
inline HP_AudioClip::RawData HP_AudioClip::loadMP3(const void* data, size_t data_size)
{
    RawData result;
    drmp3_config config;
    ...
    if (!pcmData) {
        HP_INTERNAL_LOG(E, "AUDIO: Failed to decode MP3 file");
        return result;
    }
    ...
    if (config.channels == 1) {
        ...
        HP_INTERNAL_LOG(E, "AUDIO: Unsupported number of channels (%u) in MP3 file", config.channels);
...
inline HP_AudioClip::RawData HP_AudioClip::loadOGG(const void* data, size_t data_size)
{
    RawData result;
    int channels = 0;
    int sampleRate = 0;
    ...
    if (total_samples == -1 || !pcmData) {
        HP_INTERNAL_LOG(E, "AUDIO: Failed to decode OGG file");
        return result;
    }
    ...
    if (channels == 1) {
        ...
        HP_INTERNAL_LOG(E, "AUDIO: Unsupported number of channels (%d) in OGG file", channels);
        SDL_free(pcmData);
        return result;
    }
...
inline void HP_AudioClip::cleanupRawData(RawData& audioData)
{
