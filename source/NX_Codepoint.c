/* NX_Codepoint.cpp -- API definition for Nexium's codepoint module
 *
...
int NX_GetCodepointNext(const char* text, int* codepointSize)
{
    const char* ptr = text;
...
int NX_GetCodepointPrev(const char* text, int* codepointSize)
{
    const char* ptr = text;
    int codepoint = 0x3f;       // Codepoint (defaults to '?')
    int cpSize = 0;
    *codepointSize = 0;
    ...
    codepoint = NX_GetCodepointNext(ptr, &cpSize);
...
int NX_GetCodepointCount(const char* text)
{
    uint32_t length = 0;
    const char* ptr = text;
...
const char* NX_CodepointToUTF8(int codepoint, int* utf8Size)
{
    static char utf8[6];
    SDL_memset(utf8, 0, 6);
    int size = 0;
...
int NX_GetCodepointFromUTF8(const char* text, int* codepointSize)
{
    /*
...
char* NX_ConvertCodepointsToUTF8(const int* codepoints, int length)
{
    ...
    for (int i = 0, bytes = 0; i < length; i++) {
        utf8 = NX_CodepointToUTF8(codepoints[i], &bytes);
