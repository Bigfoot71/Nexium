...
NX_ModelAnimation** PoolModel::loadAnimationsFromScene(const aiScene* scene, int* animCount, int targetFrameRate)
{
    ...
    NX_ModelAnimation** animations = static_cast<NX_ModelAnimation**>(SDL_calloc(scene->mNumAnimations, sizeof(NX_ModelAnimation*)));
    ...
    if (successCount < scene->mNumAnimations) {
        NX_INTERNAL_LOG(W, "RENDER: Only %d out of %d animations were successfully loaded", successCount, scene->mNumAnimations);
...
bool PoolModel::loadModelFromScene(const aiScene* scene, NX_Model* model)
{
    /* --- Process materials --- */

    if (!processMaterials(model, scene)) {
        NX_INTERNAL_LOG(E, "RENDER: Failed to process materials; The model will be invalid");
        return false;
    }
    ...
    if (!processMeshes(model, scene, scene->mRootNode)) {
        NX_INTERNAL_LOG(E, "RENDER: Failed to process meshes, model will not be animated");
        return false;
    }
    ...
    if (!processBones(model, scene)) {
        NX_INTERNAL_LOG(W, "RENDER: Failed to process bones, model will not be animated");
    }

    /* --- Calculate model bounding box --- */

    NX_UpdateModelAABB(model, false);
