...
NX_Vec3 interpolateAnimationKeysVec3(const aiVectorKey* keys, uint32_t numKeys, float time)
{
...
NX_Quat interpolateAnimationKeysQuat(const aiQuatKey* keys, uint32_t numKeys, float time)
{
    /* --- Case where there is only one key --- */
...
bool PoolModel::processAnimation(NX_ModelAnimation* animation, const aiScene* scene, const aiAnimation* aiAnim, int targetFrameRate)
{
    /* --- Validate input --- */
